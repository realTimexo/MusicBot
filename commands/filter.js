...
module.exports = {
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (!queue || !queue?.playing) return interaction?.reply({ content: '⚠️ Şuan Müzik çalmıyor!!', ephemeral: true }).catch(e => { })
      ...
      .setDescription('** Ritmi Keşfedin, Aşağıdan Ses Sihrinizi Seçin!**')
...
if(!filters?.includes(button?.customId)) return
      ...
      if (!filtre) return interaction?.editReply({ content: '❌ Geçersiz isim', ephemeral: true }).catch(e => { })
